...
<!-- Still absent: the OpenSSF Best Practices badge. It is the one remaining
     item that needs an interactive signup, and the passing-level questionnaire
     is already answered against this repository in
     docs/openssf-best-practices.md — submitting it is what produces the
     project ID the badge URL needs. -->
...
| Search | `find`, `find_free_space`, `Needle` | One primitive over a `Needle::{Bytes, Word, FreeRun}`; every named finder is an overload of it. Each call takes a `start` and returns the match offset, so searches compose by nesting |
...
| Install a branch | `install_branch`, `verify_branch`, `InstallMismatch` | The whole detour-installation pattern — encode, write, decode back to confirm — in one call, so a patch that did not land is an error and not a surprise later |
