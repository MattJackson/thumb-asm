...
fn push_with_a_reglist_that_overflows_its_field_is_rejected_not_encoded() {
    ...
    assert_eq!(
        ...
        0xB500u16.to_le_bytes()
    );
    assert_eq!(
        accepted_hw(|a| {
            a.push(0x0103);
        }),
        0xB503u16.to_le_bytes()
    );
    assert_eq!(
        accepted_hw(|a| {
            a.pop(0x0100);
...
fn push_and_pop_accept_the_widest_list_the_encoding_can_hold() {
    ...
    assert_eq!(
        ...
        0xB5FFu16.to_le_bytes()
    );
    assert_eq!(
        accepted_hw(|a| {
            a.pop(0x01FF);
        }),
        0xBDFFu16.to_le_bytes()
    );
    // One bit past the field is still refused.
    assert!(rejected(|a| {
        a.push(0x0200);
    })
    .contains("outside R0-R7"));
    assert!(rejected(|a| {
        a.pop(0x0200);
    })
    .contains("outside R0-R7"));
...
fn immediate_fields_reject_oversized_and_unaligned_values() {
    ...
    .contains("exceeds maximum 62"));
    assert!(rejected(|a| {
        a.strh_imm(0, 1, 1);
    })
    .contains("multiple of 2"));
