<!--
This file is the Contributor Covenant v2.1, reproduced verbatim. It is not this
project's work and is not under this project's licence: the Covenant is
published under CC BY 4.0 by its authors, and reproducing it here keeps that
licence and that attribution rather than relabelling someone else's text as
ours. The same reasoning as LICENSES/LicenseRef-Arm-Documentation.txt applies
to the Arm manuals in spec/.
...
community leader responsible for enforcement, Matthew Jackson, at <matthew@pq.io>.

This is a single-maintainer project, so a report goes to one person. If the report is *about* that
person, say so plainly in the message — it will still be read and answered, and you are also free
to raise it in public on the issue tracker instead. See [`GOVERNANCE.md`](GOVERNANCE.md) for what
single-maintainer means here.

Do not use the process in [`SECURITY.md`](SECURITY.md) for a conduct report. That channel is for
vulnerability disclosure, not conduct.
